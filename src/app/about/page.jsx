...
import { MapPin, Mail, Phone, Sun, Moon, Clock, Award, Users, Globe } from 'lucide-react'
    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
      ...
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="flex flex-col space-y-8 pb-10 pt-16 md:pt-24">
          <div className={`max-w-max rounded-full ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-blue-50 border-blue-100'} border p-1 px-3`}>
            <p className="text-xs font-semibold leading-normal md:text-sm">About Our Company</p>
          </div>
          <h1 className={`text-3xl font-bold md:text-5xl md:leading-tight ${darkMode ? 'text-white' : 'text-gray-900'}`}>
            Made with <span className="text-red-500">♥</span>, right here in <span className={`${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>India</span>
          </h1>
          <p className={`max-w-4xl text-base md:text-xl ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-12">
          <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-blue-50'} transition-transform duration-300 hover:scale-105`}>
            <Award className={`h-10 w-10 mb-4 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
          ...
          <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-blue-50'} transition-transform duration-300 hover:scale-105`}>
            <Users className={`h-10 w-10 mb-4 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
          ...
          <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-blue-50'} transition-transform duration-300 hover:scale-105`}>
            <Globe className={`h-10 w-10 mb-4 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} />
        ...
        <div className="my-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-50'} flex flex-col gap-3 transition-all duration-300 hover:shadow-lg`}>
          ...
          <div className={`p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-gray-50'} flex flex-col gap-3 transition-all duration-300 hover:shadow-lg md:col-span-2 lg:col-span-3`}>
        ...
        <hr className={`my-12 ${darkMode ? 'border-gray-700' : 'border-gray-200'}`} />

        {/* Contact Us Section */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-10 py-12">
          <div className="md:w-1/2">
            <div className={`inline-block px-4 py-2 rounded-full ${darkMode ? 'bg-blue-900/50 text-blue-400' : 'bg-blue-100 text-blue-800'} font-medium text-sm mb-6`}>
              Contact Us
            </div>
            
            <p className={`text-base ${darkMode ? 'text-gray-300' : 'text-gray-600'} md:text-lg mb-6`}>
            ...
            <div className="flex gap-4">
              <a 
                ...
                className={`inline-flex items-center gap-2 rounded-lg ${darkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-600 hover:bg-blue-700'} px-4 py-3 text-sm font-medium text-white transition-colors`}
              >
                <Mail size={18} />
                Send Email
              </a>
